...
<body>
    ...
    <c:if test="${not empty allMember}">
        <table>
            ...
            <c:forEach items="${allMember.content }" var="member">
         ...
         <div class="pagination-container">
	        <form:form method="get" action="/searchMember" modelAttribute="allMember">
	            <div class="pagination">
	                <c:if test="${not allMember.first}">
	                    <a href="?page=${allMember.number - 1}">&lt;</a>
	                </c:if>
						<c:forEach begin="${Math.max(0, allMember.number - 5)}" end="${Math.min(allMember.totalPages - 1, allMember.number + 5)}" step="1" varStatus="loop">
						    <c:choose>
						        <c:when test="${loop.index eq allMember.number}">
